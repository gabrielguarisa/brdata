...
def test_crawler_finds_correct_link(mocker):
    ...
    mock_get = mocker.patch("brdata.cvm.requests.get")
    mock_get.return_value.status_code = 200
    mock_get.return_value.text = html_content

    result = cvm.crawler("http://link-falso.com", "2024")
...
def test_dataset_raises_exception_when_not_found(mocker):
    ...
    mocker.patch("brdata.cvm.crawler", return_value=None)

    with pytest.raises(Exception, match="Download Failure"):
        cvm.dataset(2025, "ITR")
...
def tests_datasets_in_range_calls_dataset_multiple_times(mocker):
    ...
    mock_dataset = mocker.patch("brdata.cvm.dataset")
    mocker.patch("brdata.cvm.tqdm", lambda x, **kwargs: x)

    cvm.datasets_in_range("ITR", start_year=2021, last_year=2023)
...
def test_crawler_no_link_found(mocker):
    ...
    assert cvm.crawler("http://url.com", "2023") is None


# 5. Teste do Crawler - Erro de Conexão
def test_crawler_connection_error(mocker):
    mocker.patch(
        "brdata.cvm.requests.get", side_effect=requests.exceptions.ConnectionError
    )

    assert cvm.crawler("http://url.com", "2024") is None
...
def test_datasets_in_range_stop_on_exception(mocker):
    mocker.patch("brdata.cvm.dataset", side_effect=Exception("Fatal Error"))
    mocker.patch("brdata.cvm.tqdm", lambda x, **kwargs: x)
...
def test_dataset_overwrite_behavior(mocker):
    mocker.patch("brdata.cvm.os.path.isfile", return_value=True)
    mocker.patch("brdata.cvm.os.makedirs")
    mocker.patch("brdata.cvm.crawler", return_value="arquivo.zip")

    mock_get = mocker.patch("brdata.cvm.requests.get")

    # Teste 1: Não deve baixar se já existe e overwrite é False
    cvm.dataset(2023, "FCA", overwrite=False)
    ...
    cvm.dataset(2023, "FCA", overwrite=True)
